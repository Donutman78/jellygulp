...
async def analytics(days: int = Query(default=30, ge=1, le=90)):
    ...
    year_since = datetime.now(timezone.utc) - timedelta(days=365)

    with SessionLocal() as db:
        events = db.scalars(
            select(PlaybackEvent)
            .where(PlaybackEvent.occurred_at >= since)
            .order_by(PlaybackEvent.occurred_at)
        ).all()
        year_events = db.scalars(
            select(PlaybackEvent)
            .where(PlaybackEvent.occurred_at >= year_since)
            .order_by(PlaybackEvent.occurred_at)
        ).all()

    result = build_analytics(list(events), days)
    result["yearly_top_users"] = build_analytics(list(year_events), 365)["top_users"]
    return result
